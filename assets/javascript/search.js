...
function displaySearchResults(results, store, term) {
    var searchPageHeader = document.getElementById('search-page-header')
    var searchResults = document.getElementById('search-results');

    searchPageHeader.innerHTML = "Search results for '" + term + "':";
    ...
    if (results.length) { // Are there any results?
      ...
      for (var i = 0; i < results.length; i++) {  // Iterate over the results
        ...
        appendString += '<li><h5><a href="' + item.url + '">' + item.title + '</a></h5>';
